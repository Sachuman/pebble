...
func (i *twoLevelIterator[I, PI, D, PD]) SeekPrefixGE(
	...
	if i.secondLevel.maximumSuffixProperty != nil && len(key) > len(prefix) && i.secondLevel.readEnv.Bounds != nil {
		smallest := i.secondLevel.readEnv.Bounds.SmallestUserKey()
		smallest = i.secondLevel.reader.Comparer.Split.Prefix(smallest)
		largest := i.secondLevel.readEnv.Bounds.LargestUserKey()
		...
		if i.secondLevel.cmp(prefix, smallest) > 0 && i.secondLevel.cmp(prefix, largest) < 0 {
			...
			if i.secondLevel.transforms.HasSyntheticSuffix() {
				maxSuffix = i.secondLevel.transforms.SyntheticSuffix()
				ok = true
			} else {
